...
namespace Pasantes.Website.Controller
{
    public class DefaultController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult BodyRight()
        {
            var item = _renderingService.GetDataSource<BodyRight>();

            return View("~/Views/BodyRight.cshtml", item);
        }

        public ActionResult BodyLeft()
        {
            var item = _renderingService.GetDataSource<BodyLeft>();


            return View("~/Views/BodyLeft.cshtml", item);
        }
    }
}